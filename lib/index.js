...
const takeRandomQuiz = (output, n, quizzes) => Promise.all(readMultipleFiles(quizzes)).then(data => data.reduce((all, quiz) => all.concat((0, _lib.chooseRandom)((0, _lib.createQuestions)(quiz), n)), [])).then(quizData => quizData.map((q, i) => renameValue(q, i)))
// .then(data => console.log(data))
.then(quizData => (0, _inquirer.prompt)(quizData)).then(randomQuiz => (0, _lib.writeJsonFile)(`quiz/answers/${output}.json`, JSON.stringify(randomQuiz))).catch(err => console.log('Error taking the random quiz.', err));
