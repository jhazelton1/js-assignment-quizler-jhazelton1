...
const readMultipleFiles = arr => arr.reduce((all, file) => {
...
const takeQuiz = (title, output) => (0, _lib.readJsonFile)(`quiz/${title}.json`).then(data => (0, _lib.createQuestions)(JSON.parse(data))).then(quizData => (0, _inquirer.prompt)(quizData)).then(answers => (0, _lib.writeJsonFile)(`quiz/answers/${title}-${output}.json`, JSON.stringify(answers))).catch(err => console.log('Error taking the quiz.', err));
...
const concatQuizzes = (quizData, n) => {
  return quizData.reduce((all, quiz) => all.concat((0, _lib.chooseRandom)((0, _lib.createQuestions)(quiz), n)), []);
};

const takeRandomQuiz = (output, n, quizzes) => Promise.all(readMultipleFiles(quizzes)).then(data => concatQuizzes(data, n)).then(quizData => quizData.map((q, i) => renameValue(q, i))).then(quizData => (0, _inquirer.prompt)(quizData)).then(randomQuiz => (0, _lib.writeJsonFile)(`quiz/answers/${output}.json`, JSON.stringify(randomQuiz))).catch(err => console.log('Error taking the random quiz.', err));
