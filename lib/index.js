...
const takeRandomQuiz = (output, n, quizzes) => Promise.all(readMultipleFiles(quizzes)).then(data => concatQuizzes(data, n)
// data.reduce(
//   (all, quiz) => all.concat(chooseRandom(createQuestions(quiz), n)),
//   []
// )
).then(data => console.log(data)).then(quizData => quizData.map((q, i) => renameValue(q, i))).then(quizData => (0, _inquirer.prompt)(quizData)).then(randomQuiz => (0, _lib.writeJsonFile)(`quiz/answers/${output}.json`, JSON.stringify(randomQuiz))).catch(err => console.log('Error taking the random quiz.', err));
